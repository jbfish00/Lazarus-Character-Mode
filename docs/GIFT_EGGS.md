...
itself is pinned by five checks in `verify_artifacts.py` (133), negative-tested
6/6 by `tools/tests/egg_hook_negative_test.py`.

✅ **THE HOOK IS IN** (`tools/character_mode/egg_hook.py`). The hatch script's
tail at `0x08349169` is overlaid with a `goto` into an 11-byte replayed tail at `0x09670000`
...
exemption inside the sweep no longer applies to it. Build `2a8a7e5a`.
