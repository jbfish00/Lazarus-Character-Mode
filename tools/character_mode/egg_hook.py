...
(docs/GIFT_EGGS.md, ../game_plans/rowe_parity.md §13.18): 3 reachable `giveegg` sites -- the hot-spring EGG (Wynaut, off-roster for ALL 123 offered characters) and two Rare Egg NPCs rolling `random 66` and `random 34` over VAR 0x800D, 97% off-roster for the median character.

RE summary (2026-09-03). Hatching is SCRIPT-DRIVEN, which is what makes this
cheap. The field-control step handler runs the script at 0x0834915D when
`ShouldEggHatch` returns true -- its pointer sits in that caller's literal pool
at 0x0812C664, and it decodes to:

    0x0834915D: 69                 lockall
              +1: 0F 00 <text>      loadword 0, "Huh?"
              +7: 09 04             callstd MSGBOX_DEFAULT
    0x08349169: 25 C5 00           special EggHatch  <- performs the hatch
              +3: 27                 waitstate
              +4: 6B                 releaseall
              +5: 02                 end
...
- **Nothing references the interior of the script.** An UNALIGNED u32 scan of
  the whole ROM finds 0x0834915D (the entry) exactly once -- the caller's literal pool at 0x0812C664, and finds ZERO
...
SCRIPT_ENTRY = 0x834915d
CALLER_POOL_OFF = 0x12c664          # file offset of the caller's literal pool
SPLICE_ROM_ADDR = 0x8349169
SPLICE_FILE_OFF = 0x349169
...
if __name__ == "__main__":
    blob, patches = build(0x9670000, 0x08000001)
