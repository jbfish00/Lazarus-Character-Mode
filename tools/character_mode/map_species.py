...
NAME_FIXES = {
    ...
    # INVERSE truncation fixes. The cross-game audit overlays were generated in
    # ROWE, whose emitter truncates species names to 10 characters, so names
    # arrive here in ROWE's clipped spelling and match nothing in this donor --
    # silently dropping the species rather than erroring. Every game truncates
    # differently, so this table is necessarily per-repo. All 18 below were
    # confirmed to resolve in THIS donor before being added; "Scatterbug" also
    # arrives unmatched but is genuinely absent here, so it is deliberately not
    # listed (mapping it would invent a species this game does not have).
...
def merge_additions(raw):
    ...
    for char_name, extra in additions.items():
        ...
        # carries each add's provenance alongside it. Assuming bare strings
        # crashes on `s not in have` with "unhashable type: 'dict'" the moment
        # the audit overlay lands -- a documented per-repo defect, present in
        # every game so far.
        extra_names = {e["species"] if isinstance(e, dict) else e for e in extra}
...
REGION_PREFIXES = ("Alolan", "Alola", "Galarian", "Galar",
                   "Hisuian", "Hisui", "Paldean", "Paldea")


def regional_fallback(name, name_to_const):
    """Resolve "Hisuian Arcanine" to plain Arcanine's constant.

    This donor (pokeemerald-expansion family headers) carries **no** regional
    names at all -- 0 of its 1013 display names mention Alola/Galar/Hisui/Paldea
    -- so a prefixed Bulbapedia name matches nothing and the species is silently
    dropped. That is not cosmetic: it costs Volo his only Fire-type and maps
    Palina, whose entire roster is Hisuian forms, to an EMPTY roster.

    A regional variant is still that species, and the project's family rule says
    a family is canon if any member is, so falling back to the plain species is
    the correct answer here rather than a lossy approximation. (Radical Red's
    equivalent prefers a real regional constant when its donor has one and only
    then falls back; this donor never has one.) Returns None if the name is not
    regional or the base species is unknown.
    """
    parts = name.split(" ", 1)
    if len(parts) != 2 or parts[0] not in REGION_PREFIXES:
        return None
    return name_to_const.get(parts[1])


def load_removals():
    """The audited-away species from roster_removals.json, raw.
    ...
    if not os.path.isfile(path):
        return {}
    with open(path, encoding="utf-8") as f:
        return json.load(f)["removals"]


def load_audit_keeps():
    """Species an audit wave explicitly KEPT, which outrank another wave's
    removal of a different member of the same family.

    Without this the family rule inverts: Lana's Milotic is another trainer's
    and her Feebas is her own, so the Milotic verdict would take the Feebas --
    and the whole family -- with it.
    """
    path = os.path.join(HERE, "audit_keeps.json")
    if not os.path.isfile(path):
        return {}
    with open(path, encoding="utf-8") as f:
        return json.load(f).get("keeps", {})


def merge_removals(raw, removals):
    """The CHEAP HALF of applying removals: subtract the exact names listed.

    THE FAMILY RULE (user, 2026-07-25): a full evolution family is allowed
    whenever any single member of it is canon, forwards and backwards. This pass
    alone does NOT enforce that, and the docstring that used to sit here claimed
    it did ("the file lists only species whose ENTIRE family was removed -- so
    plain subtraction is correct"). It is not: the scraper often lists a later
    stage under its own name, and that stage walks the family straight back in
    once names are canonicalized to family bases. Radical Red hit exactly this --
    Leaf's Charmander was removed while "Charizard" stayed in her raw list, and
    she kept the whole line.

    The half that actually enforces the rule is the family-level sweep in main(),
    which runs after canonicalization and is shielded by audit_keeps.json.
    """
    dropped = 0
    ...
    print("roster_removals.json: dropped %d species by name across %d characters"
...
def main():
    ...
    def resolve_const(name):
        ...
        if const is None:
            const = regional_fallback(fixed, name_to_const)
        return const

    # The other half of the family rule. A removal is a verdict on the whole
    # FAMILY, not on the single name the auditor happened to be shown, so it has
    # to be re-applied here -- after names collapse to family bases -- or a later
    # stage listed under its own name walks the family back in.
    #
    ...
    # user's rule, both directions). Hence the shield.
    audit_keeps = load_audit_keeps()
    family_removed = {}
    shielded = 0
    for char, rows in removals.items():
        kept_bases = set()
        for name in audit_keeps.get(char, ()):
        ...
        for r in rows:
            ...
            if const is None:
                continue
            base_const = base.get(const, const)
            if base_const in kept_bases:
                shielded += 1
                continue
            bases.add(base_const)
        family_removed[char] = bases
    swept = 0
    ...
    for disp, info in sorted(raw.items()):
        ...
        for name in info["species"]:
            ...
            if const is None:
                ...
                continue
            base_const = base.get(const, const)
            if base_const in family_removed.get(disp, ()):
                swept += 1
                continue
            consts.add(base_const)
    ...
    if shielded:
        print("roster_removals.json: %d held back by an audit keep on the same "
              "family" % shielded)
