...
def build(tmp):
    ...
    # lives in that region: when it was added and this line was not updated,
    # the checker died with NameError in the fake tree -- and because a crash
    # also exits non-zero, FIVE tamper cases still reported PASS. Only the
    # CONTROL caught it. A tamper case that cannot tell a crash from a
...
def t_new_sibling_ref(repo):
    ...
    reached tools/remove_pride_flags.py past checks [1]-[5], which only ever
    looked for the ROWE path."""
    p = os.path.join(repo, "tools", "new_tool.py")
    with open(p, "w") as f:
        f.write('import sys, os\n'
                'sys.path.insert(0, os.path.join(os.path.dirname(__file__),\n'
                '    "..", "..", "Unbound-Character-Mode", "tools"))\n'
                'from lz77 import decompress\n')
    return os.path.exists(p) and "Unbound-Character-Mode" in open(p).read()
